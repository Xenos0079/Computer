...
def create_square():
    """
    create two paired turtle one for the block and the other for number
    Return:
    b:The turle for the block
    n:The paired turle for the text
    """
    sz=4
    t_1=turtle.Turtle()
    t_1.hideturtle()
...
def is_solvable(number_list):
    """
    To check if the generated game can be solved

    Arg:
    number_list:the list of the text of the number
...
def create_game(dim,space):
    ...
    Arg:
    dim: The dimension of the game(3*3,4*4,5*5)
    space: The distance of two blocks

    Return:
    b_g:the list of turtle for block 
    n_g:the list of turtle for the next
    number_list:the list of numebr for the next
...
def check_game(number_list,dim):
    """
    To check if the game has been solved

    Arg:
    number_list"the list of the text of the game
    dim: the dimension of the game

    Return:
    1 for has solve the game
    0 for has not solve the game
    """
...
def change_color(b_g,n_g,number_list):
    """
    To change all block to red after solve the game
...
def touch(x,y):
    """
    To let the block slide and change the position

    Arg:
    x,y: the position where theb mouse click
    """
...
dim=int(turtle.numinput('Square Puzzle','Puzzle Dimension'))
