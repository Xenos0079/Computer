...
def configure_play_area():
    ...
    it_w , st_w = create_turtle(-160 , 115) , create_turtle(0,0)
    it_w.hideturtle()
    st_w.hideturtle()
...
def random_position():
    ...
    if out_of_bound(x , y) or [x  , y] in g_postion_set or (x ** 2 + y ** 2) * 0.5 < 100:
...
def o_t_s(): # o_t_s
    ...
    if skip():
        return
    global g_head_positions, g_press_history
    if g_key_pressed is None \
        or (g_key_pressed == KEY_SPACE and g_press_history[-2] == KEY_SPACE): # should be still
        check_collision()
        update_status()
        g_screen.update()
        g_screen.ontimer(o_t_s , TIMER_SNAKE)
        return
    # snake is moving
    ...
    update_status() # 1/
    g_screen.update() # 2/
    g_screen.ontimer(o_t_s , TIMER_SNAKE) # 3/ 
...
def o_t_m(): # on_timer_monster
    ...
    global count
    if g_key_pressed is None:
        g_screen.ontimer(o_t_m , TIMER_SNAKE)
        return
    for monster in g_monsters:
        ...
        monster.forward(random.randint(SZ_SQUARE * 2 , SZ_SQUARE * 3)) # set the moving distance
    chaotic_case(TIMER_MONSTER , o_t_m)

def o_t_f(): # on_timer_food
    '''
    ...
    if g_key_pressed is None: # no input
        g_screen.ontimer(o_t_f , TIMER_SNAKE)
        return
    for food in g_foods:
...
def check_collision():
    ...
    for monster in g_monsters:
        if g_head.distance(monster) < SZ_SQUARE: # head / monster
            endgame()
        if any(monster.distance(pos) < SZ_SQUARE for pos in g_head_positions): # body / monster
            count += 1
            update_status()
    cor_edge = DIM_PLAY_AREA // 2 - SZ_SQUARE
    if abs(g_head.xcor()) >= cor_edge or abs(g_head.ycor()) >= cor_edge: # head / edge
        g_key_pressed = KEY_SPACE
        update_status()
    for food , i in g_foods:
        if g_head.distance(food) < SZ_SQUARE: # head / food
...
def cb_s(x ,y): # configure_start_game
    '''
    ...
    for key in [KEY_UP , KEY_DOWN , KEY_LEFT , KEY_RIGHT , KEY_SPACE]:
        g_screen.onkey(partial(pressed,key) , key)

    o_t_s() # 1 /
    o_t_m() # 2 /
    o_t_f() # 3 /
    # 1/2/3 set the timer for the objects 
...
def c_m(): # create_monster
    '''
    Creates the monsters on the screen.
    '''
    g_monsters = []
    for _ in range(1 , NUM_MONSTER + 1):
        x , y = random_position()
...
def main():
    ...
    g_monsters = c_m()
    g_foods = c_f()
    g_screen.onscreenclick(cb_s) # set up a mouse-click event
    g_screen.update()
    g_screen.listen()
    g_screen.mainloop()
